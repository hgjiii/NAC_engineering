...
def write_ligand_kinemage(f, molfile): #{{{
    ...
    f.close()
#}}}
def write_param_file(f, molfile, output_dir, name, frag_id, base_confs, max_confs, amino_acid=None,long_names=False,conformer_file=""): #{{{
    ...
    close_file = False
    if isinstance(f, str):
        f = gz_open(os.path.join(output_dir, f), 'w')
        close_file = True
...
#}}}
def write_ligand_pdb(f, molfile_tmpl, output_dir, molfile_xyz, resname, ctr=None, chain_id='X'): #{{{
    ...
    f may be a file name or file handle.'''
    if isinstance( f, str): f = gz_open(os.path.join(output_dir, f), 'w') 
        ...
        if options.conformers_in_one_file:
            ...
            for i, molfile in enumerate(molfiles):
                ...
                if not options.clobber and os.path.exists(pdb_file):
                    ...
                    try:
                        write_ligand_pdb(pdb_file, m, options.output_dir, molfile, options.name, options.center, options.chain)
                    except ValueError as e:
    ...
    if not options.no_param:
        for i in range(num_frags):
            ...
            if not options.clobber and os.path.exists(param_file):
                ...
                write_param_file(param_file, m, options.output_dir, options.name, i+1, len(molfiles), options.max_confs, options.amino_acid, long_names=options.long_names, conformer_file=conformer_file)
