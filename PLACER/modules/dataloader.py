...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataframe, params):
        ...
        '''
        self.dims1d = (118+1, # element type, 0=unknown
                       params['maxcharge']*2+1, 1, # formal charge and mask
                       params['maxhydr'], 1, # number of hydrogens and mask
    ...
    def get_crop_center(self, chains):
    ...
    def get_crop_around_mol(self, chains, obmol):
        
        # TODO: this function can probably be merged with the one below it
        ...
        #res = np.array(list(cropped_dict.keys()))
        #naa = np.array(list(cropped_dict.values()))
        #cropped_res = set([tuple(r) for r in res[naa.cumsum()<maxatoms]])
    ...
    def get_crop(self, chains, center):
        ...
        if type(center[0])==float:
            ...
            matches = [(i,d) for p in center for d,i in zip(*kd.query(p,maxatoms))]
    ...
    def get_features(self, G):
        
        N = len(G)
        
        ##### 1D #####
        atoms = [attr['atom'] for node,attr in G.nodes(data=True)]
        f1d = torch.tensor([(a.element,a.charge,a.nhyd,a.hyb) for a in atoms])
    ...
    def __getitem__(self, index):
        ...
        #f1d = self.OneHotF1D(f1d,qmask=qmask,hmask=hmask)
